...
if [[ $count -lt  60 ]] ; then
    ...
    curl -d '{ "branch" : ''"'${lastBranch}'"'', "service": ''"'${serviceName}'"''}'  -H "Content-Type: application/json" -X POST http://172.19.125.135:8088/recordBranchInProd # update the svn record for prod env
    updateJob --service  ${serviceName} --branch ${lastBranch} #update the svnpath in its jenkins job.
