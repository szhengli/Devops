...
function deploy_to_k8s(){
  if [[ $# -eq 2 ]] ; then
    ...
    image=$(kubectl get deploy ${service}  -n ${ENV}gray  --context gray  -o jsonpath={.spec.template.spec.containers[0].image} )
    if [ -n "$image" ] ;
    then
         branch=$(kubectl get deploy ${service}  -n ${ENV}gray  --context gray  -o jsonpath={.metadata.labels.version} )
    else
         echo "not find in new"
         image=$(kubectl get deploy ${service}  -n ${ENV}gray  --context prodv5  -o jsonpath={.spec.template.spec.containers[0].image} )
         branch=$(kubectl get deploy ${service}  -n ${ENV}gray  --context prodv5  -o jsonpath={.metadata.labels.version} )
    fi      

    if echo ${image} | grep -q "prodv5gray" ; then
      echo "the image find in gray namespace is valid"
    else
