...
then
   #分支存在，判断分支下这个项目是否存在
   if ! svn info $auth ${srcBranchUrl}/${project} &>/dev/null
   then
      echo "svn分支迁移失败: ${srcBranchUrl}/${project} 不存在，无法迁移请确认该分支下项目是否存在。"
      exit 0
   fi
else
   echo   "svn分支迁移失败：${srcBranchUrl} 分支不存在，无法迁移!"
   #程序正常提前退出
   exit 0
fi
...
then
   ...
   svn rm  $auth  $srcBranchUrl/${project} -m "${comment}" &>/dev/null 
   eval sed -i  "'/"$srcBranch"\/"$project"/,+2d'" /data/svn/authz
else
   if bash   /data/script/branchRo.sh $srcBranchUrl/${project} &>/dev/null
   then
      echo  "svn分支迁移成功: 从 ${srcBranchUrl}/${project} 到 ${desBranchUrl}/${project}。"
   else
