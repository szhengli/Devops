...
then
   #新项目，需运维人员人工创建。
   echo   "svn分支：$project 为新项目，请尽快联系运维手动创建此新项目，并另行通知。"
    #程序正常提前退出
   exit 0  
fi
...
if [ ! `date -d $branch +%Y%m%d` ]
then
   #分支不符合规范，请使用正常的分支
   echo "svn分支: ${branch}不符合日期规范，请重新申请。"
   exit 0
fi
...
#     echo "exist"
      ...
      svn copy $auth  $source_path $des_path -m "${comment}" &>/dev/null  && \
