...
#目的SVN路径
des_path=http://192.168.3.144/svn/zlnet/code/project/branch/$year/$month/$branch/
...
if [[ ! -n `svn list http://192.168.3.144/svn/zlnet/code/project/branch/$year/$month | grep $branch` ]]; then
			...
			echo -e "\n[zlnet:/code/project/branch/$year/$month/$branch]\n*=r\nsvnadmin=rw" >> /data/svn/authz
			svn copy $source_path $des_path -m "create a private $branch of $project" --username svnadmin --password Zhonglun@2020 && \
