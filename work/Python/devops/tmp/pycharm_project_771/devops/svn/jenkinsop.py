...
JENKINS_CONN = {"url": "http://172.19.125.135:8080/", "username": "admin", "password": "admin"}
...
# to update the svn path of the prod jenkins job by the one of synced gray job. to enable the urgent unlock request
def  updateSyncedJobSvn(sysops, branch):
    target = "生产环境"
    services = re.split('[ \n,、，；;]+', sysops.strip().lower())
    for service in services:
        SVNUpdate(service, branch, target)
        print(service + "原发布job中分支路径已经更新")
    print(sysops + "原发布job中分支路径已经全部更新完毕！！！！")
...
def buildJob(service, types):
    jk = Jenkins(**JENKINS_CONN)
    if types == "restart":
        jobName = "prodv5-prodv5" + "-" + service + "-restart"
    elif types == "rollback":
        jobName = "prodv5-prodv5"  + "-" + service + "-restart"
    jk.build_job(jobName)
