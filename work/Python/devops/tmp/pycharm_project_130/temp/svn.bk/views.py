...
def webUpdateJOBSvnPath(request):
    if request.method == 'POST':
        ...
        return HttpResponse(service +" svn.bk path in jenkins Job changed to" + branch + " !")
    else:
...
def checkClass(project):
    ...
    for cat in catalogs:
        projects = getstatusoutput("svn.bk list " + parent + cat + auth)[1].split('\n')
